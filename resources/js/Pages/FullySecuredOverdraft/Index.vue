...
<template>
    <AppLayout :nav-urls="navUrls">
        <div class="p-6">
            ...
            <h1 class="text-xl font-semibold cvr-text-primary mb-1">{{ $t('Fully Secured Overdraft') }}</h1>
                    ...
                    <tbody>
                        ...
                        <tr v-if="filteredRows.length === 0">
                            <td colspan="10" class="px-4 py-8 text-center cvr-text-muted">
                                {{ $t('No Fully Secured Overdraft records found.') }}
                            </td>
                        </tr>
                    </tbody>
                </table>
                                    ...
                                    <table class="w-full text-xs border rounded overflow-hidden">
                                        <thead class="cvr-table-head">
                                            <tr>
                                                ...
                                                <th class="px-3 py-2 text-start">{{ $t('Lending %') }}</th>
            ...
            <div v-if="renewTarget" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                <div class="cvr-modal rounded-lg p-6 w-full max-w-2xl">
                    ...
                    <div class="cvr-form-grid-2 mb-4">
                        ...
                        <div>
                            <label class="block text-xs cvr-text-secondary mb-1">{{ $t('New CD Or TD Lending Percentage (%)') }}</label>
                            ...
                            <p v-else class="text-xs cvr-text-muted mt-1">{{ $t('Limit will be:') }} <strong>{{ previewedRenewalLimit?.toLocaleString('en-EG') }}</strong> {{ $t('(linked CD/TD amount × this percentage)') }}</p>
                        </div>
                        <div>
