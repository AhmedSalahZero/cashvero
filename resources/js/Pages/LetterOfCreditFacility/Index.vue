                        ...
                        <tr v-for="(row, index) in filteredRows" :key="row.id" class="cvr-table-row">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <button @click="openRenew(row)" class="cvr-btn-secondary inline-flex items-center px-2 py-1 rounded border text-xs" title="Renew this facility with new terms">
                                        Renew
                                    </button>
                                    ...
                                    <div v-for="(chapter, cIndex) in row.terms_history" :key="chapter.id" class="mb-4">
                                        <div class="flex items-center justify-between mb-2">
                                            <p class="text-xs cvr-text-muted">
                                                ...
                                                — Borrowing {{ chapter.borrowing_rate }}% + Margin {{ chapter.bank_margin_rate }}% = Interest {{ chapter.interest_rate }}%
                                            </p>
                                            <button
            ...
            <div v-if="deleteRenewalTarget" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                <div class="cvr-modal rounded-lg p-6 w-full max-w-sm">
                    ...
                    <p class="text-xs cvr-text-muted mb-4">
                        The facility will revert to its previous chapter's terms. This is blocked if any LC has
                        already been issued on or after the renewal's effective date.
                    </p>
            ...
            <div v-if="renewTarget" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                <div class="cvr-modal rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                    ...
                    <p class="text-xs cvr-text-muted mb-4">
                        ...
                        effective date is ever recalculated — past LCs keep whatever rate was in force when they were
                        issued. Provide the full new Term &amp; Conditions matrix below; it applies to any LC issued
                    ...
                    <div class="cvr-form-grid-2 mb-4">
                        <div>
                            ...
                            <p class="text-xs cvr-text-muted mt-1">Current contract end date: <strong>{{ currentContractEndDate }}</strong> — the renewal date must be after this.</p>
                    ...
                    <h3 class="text-sm font-semibold cvr-text-secondary uppercase tracking-wide mb-2">Financing Terms &amp; Conditions</h3>
                    <div class="cvr-form-grid-2 mb-4">
                        <div v-if="isFullySecuredRenewTarget">
                            <label class="block text-xs cvr-text-secondary mb-1">New CD/TD Lending Percentage (%)</label>
                            <input type="number" step="any" v-model="renewForm.cd_or_td_lending_percentage" class="cvr-input w-full" placeholder="Unchanged" />
                            <p class="text-xs cvr-text-muted mt-1">This facility is Fully Secured — the Limit is recalculated automatically from the linked CD/TD amount × this percentage.</p>
                        ...
                        <div>
                            <label class="block text-xs cvr-text-secondary mb-1">New Borrowing Rate (%)</label>
                            <input type="number" step="any" v-model="renewForm.borrowing_rate" class="cvr-input w-full" placeholder="Unchanged" />
                            <p v-if="renewErrors.borrowing_rate" class="text-xs text-red-600 mt-1">{{ renewErrors.borrowing_rate }}</p>
                        </div>
                        <div>
                            <label class="block text-xs cvr-text-secondary mb-1">New Bank Margin Rate (%)</label>
                            <input type="number" step="any" v-model="renewForm.bank_margin_rate" class="cvr-input w-full" placeholder="Unchanged" />
                            <p v-if="renewErrors.bank_margin_rate" class="text-xs text-red-600 mt-1">{{ renewErrors.bank_margin_rate }}</p>
                        </div>
                        <div>
                            <label class="block text-xs cvr-text-secondary mb-1">Interest Rate (%) — Borrowing + Margin</label>
                            <input type="text" :value="renewInterestRatePreview" disabled class="cvr-input w-full opacity-70" />
                        </div>
                        <div>
                            <label class="block text-xs cvr-text-secondary mb-1">New Min Interest Rate (%)</label>
                            <input type="number" step="any" v-model="renewForm.min_interest_rate" class="cvr-input w-full" placeholder="Unchanged" />
                        </div>
                        <div>
                    ...
                    <h3 class="text-sm font-semibold cvr-text-secondary uppercase tracking-wide mb-2">New Term &amp; Conditions — by LC Type *</h3>
