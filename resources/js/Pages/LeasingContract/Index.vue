        ...
        <div class="p-6">
            <div class="flex items-center gap-3 mb-1">
                <Link :href="backUrl" class="cvr-btn-secondary inline-flex items-center gap-1 px-3 py-1.5 rounded border text-sm">
                    {{ $t('← Back to Leasing Companies') }}
                </Link>
            </div>
            <h1 class="text-xl font-semibold cvr-text-primary mb-1">{{ $t('Leasing Contracts') }}</h1>
            <p class="text-sm cvr-text-blue mb-6">{{ leasingCompany.name }}</p>
                    ...
                    <tbody>
                        <tr v-for="(row, index) in filteredRows" :key="row.id" class="cvr-table-row">
                            ...
                            <td class="px-3 py-3">
                                <div class="flex items-center gap-1.5 flex-wrap">
                                    ...
                                    <Link v-if="row.statement_url" :href="row.statement_url" class="cvr-action-btn" :title="$t('Contract Statement')">📄</Link>
                                    <a v-if="canUpload" :href="row.upload_schedule_url" class="cvr-action-btn" :title="$t('Upload Contract Schedule')">📤💵</a>
                        ...
                        <tr v-if="filteredRows.length === 0">
                            <td colspan="11" class="px-4 py-8 text-center cvr-text-muted">
                                {{ $t('No Leasing Contract records found.') }}
                            </td>
                        </tr>
                    </tbody>
                </table>
