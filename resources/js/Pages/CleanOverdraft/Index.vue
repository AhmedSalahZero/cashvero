...
/* ── Lock / Unlock confirmation ──────────────────────────────────
   ...
   used on Bank Accounts — Clean Overdraft is one of the 7
...
const currentContractEndDate = computed(() => {
    ...
    // mirrors exactly what CleanOverdraft::renew() checks against.
...
<template>
    <AppLayout :nav-urls="navUrls">
        ...
        <div class="p-6">
            ...
            <h1 class="text-xl font-semibold cvr-text-primary mb-1">{{ $t('Clean Overdraft') }}</h1>
                    ...
                    <tbody>
                        <tr v-for="(row, index) in filteredRows" :key="row.id" class="cvr-table-row">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <RecordLogButton subject="CleanOverdraft" :id="row.id" :company-id="company.id" />
                        ...
                        <tr v-if="filteredRows.length === 0">
                            <td colspan="10" class="px-4 py-8 text-center cvr-text-muted">
                                {{ $t('No Clean Overdraft records found.') }}
                            </td>
                        </tr>
                    </tbody>
                </table>
