<script setup>
/**
 * Read-only summary for the company's Suppliers Opening Balance — a
 * singleton (one per company), not a list. Mirror of
 * CustomerOpeningBalance/Index.vue. "Manage" button leads to the
 * real create/edit form (still Blade, deliberately not migrated in
 * this pass — see SupplierOpeningBalancesController's docblock).
 */
    ...
    <AppLayout>
        <div class="p-6">
            <div class="flex items-center justify-between mb-1 flex-wrap gap-3">
                <div>
                    <h1 class="text-xl font-semibold cvr-text-primary mb-1">Suppliers Opening Balance</h1>
                    <p class="text-sm cvr-text-muted">Opening invoices and advanced down payments for this company's suppliers</p>
                </div>
            ...
            <div v-if="!exists" class="cvr-card mt-8 text-center py-12">
                ...
                <p class="text-sm cvr-text-muted mb-5">Set the starting invoices and down payments for this company's suppliers.</p>
                <a :href="manageUrl" class="cvr-btn-copper inline-flex items-center gap-1 px-4 py-2 rounded-lg text-sm font-medium">
                        ...
                        <table class="min-w-full text-sm">
                            <thead class="cvr-table-head">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left">Purchase Order #</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="row in invoices" :key="row.id" class="cvr-table-row">
                                    <td class="px-4 py-3 cvr-text-primary">
                                        <span class="block truncate max-w-[220px]" :title="row.supplier">{{ row.supplier || '—' }}</span>
                                    </td>
