                                ...
                                <tr v-for="row in tab.rows.data" :key="row.id" class="cvr-table-row">
                                    ...
                                    <td v-if="canUpdate || canDelete || canMarkAsPaid" class="px-3 py-3">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <Link v-if="!row.review?.is_reviewed && canUpdate && row.edit_url" :href="row.edit_url" class="cvr-action-btn" :title="$t('Edit')">✏️</Link>
                                            <!--
                                            ...
                                            >↩️</button>
                                            <button v-if="!row.review?.is_reviewed && canDelete && row.delete_url" @click="confirmDelete(row)" class="cvr-action-btn" :title="$t('Delete')">🗑️</button>
                                        </div>
                                    </td>
                                </tr>
