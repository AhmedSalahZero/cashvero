...
const props = defineProps({
    ...
    cdAccountTypeId: Number,
    financialInstitutionBanks: Array, // [{id, name}]
    cdAccounts: Array,                // [{id, financial_institution_id, account_number, currency, amount}]
...
const form = ref({
    ...
    cd_or_td_account_type_id: props.cdAccountTypeId,
...
const cdOptionsForBank = computed(() =>
    props.cdAccounts.filter(cd =>
        cd.financial_institution_id === Number(form.value.financial_institution_id) &&
        (!form.value.lg_currency || cd.currency === form.value.lg_currency)
    )
);
const selectedCd = computed(() => props.cdAccounts.find(cd => cd.id === Number(form.value.cd_or_td_id)));
...
watch(selectedCd, (cd) => {
    if (cd) form.value.lg_currency = cd.currency;
}, { immediate: true });
    ...
    <AppLayout :nav-urls="navUrls">
        <div class="p-6">
            ...
            <h1 class="text-xl font-semibold cvr-text-primary mb-1">
                {{ isEdit ? $t('Edit') : $t('Add') }} {{ $t('LG Issuance — Against CD') }}
            </h1>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="cvr-card">
                    ...
                    <div class="cvr-form-grid-4">
                        ...
                        <div>
                            <label class="cvr-form-label">{{ $t('CD Account #') }}</label>
                            <select v-model="form.cd_or_td_id" class="cvr-input w-full px-3 py-2 rounded">
                                <option value="" disabled>{{ $t('Select') }}</option>
                                <option v-for="cd in cdOptionsForBank" :key="cd.id" :value="cd.id">{{ cd.account_number }}</option>
                            </select>
                        </div>
                        <div>
                            <label class="cvr-form-label">{{ $t('Amount') }}</label>
                            <input disabled :value="selectedCd?.amount ?? 0" class="cvr-input w-full px-3 py-2 rounded opacity-70" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-xs cvr-text-muted mt-1">{{ $t('Already committed against this CD') }}</p>
                        </div>
                        <div>
                ...
                <!-- Commission -->
                <div class="cvr-card">
                    <h2 class="text-sm font-semibold cvr-text-secondary uppercase tracking-wide mb-4">{{ $t('Commission') }}</h2>
                    <div class="cvr-form-grid-4">
                        <div>
                        ...
                        <div>
                            ...
                            <input v-model="form.min_lg_commission_fees" type="number" step="any" class="cvr-input w-full px-3 py-2 rounded" />
                        </div>
                        <div>
                ...
                <div class="cvr-card">
                    <h2 class="text-sm font-semibold cvr-text-secondary uppercase tracking-wide mb-4">{{ $t('Fees & Commission Account') }}</h2>
                    <div class="cvr-form-grid-3">
                        <div>
