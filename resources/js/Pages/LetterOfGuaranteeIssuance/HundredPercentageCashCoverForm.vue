...
watch(
  () => [form.value.financial_institution_id, form.value.lg_currency, form.value.lg_type],
  runLookup,
  { immediate: true },
)
...
watch(
  () => [form.value.lg_amount, form.value.cash_cover_rate],
  () => {
    form.value.cash_cover_amount =
      Math.round(
        ((Number(form.value.lg_amount || 0) * Number(form.value.cash_cover_rate || 0)) / 100) * 100,
      ) / 100
  },
)
watch(
      ...
      <form
              ...
              <p class="text-xs cvr-text-muted mt-1">
                {{
                  form.lg_type === 'bid-bond'
                    ? 'Bid Bond: showing all customers'
                    : 'Showing only customers with an active contract'
                }}
              </p>
            </div>
            <div>
            ...
            <div v-if="!isBidBond">
              ...
              <select
                ...
                <option
                  ...
                  {{ contractIsRequired ? 'Select' : 'None' }}
                </option>
                <option
              ...
              <p class="text-xs cvr-text-muted mt-1">
                {{
                  isSpecificSoSelected
                    ? 'Auto-filled from the selected SO'
                    : 'Pick a date for this contract-wide LG'
                }}
              </p>
            </div>
            <div>
        ...
        <div class="cvr-card">
          ...
          <div class="cvr-form-grid-3">
            <div>
              <label class="cvr-form-label"
                >Deducted From Account # (Cash Cover &amp; Fees) *</label
              >
              <select
          ...
          <p class="text-xs cvr-text-muted mt-3">
            This source uses one account for both the cash cover deposit and issuance/commission
            fees — confirmed from the original: no separate Cash Cover account field exists for 100%
            Cash Cover.
          </p>
        </div>
