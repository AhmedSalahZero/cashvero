        ...
        <div class="p-6">
            ...
            <div class="flex items-center justify-between mb-4 flex-wrap gap-3">
                ...
                <div class="flex items-center gap-2">
                    <Link v-if="permissions.canCreate" :href="createUrls['lg-facility']" class="cvr-btn-copper px-3 py-1.5 rounded text-sm">{{ $t('+ Via LG Facility') }}</Link>
                    <Link v-if="permissions.canCreate" :href="createUrls['against-cd']" class="cvr-btn-secondary px-3 py-1.5 rounded border text-sm">{{ $t('+ Against CD') }}</Link>
                    <Link v-if="permissions.canCreate" :href="createUrls['against-td']" class="cvr-btn-secondary px-3 py-1.5 rounded border text-sm">{{ $t('+ Against TD') }}</Link>
                    ...
                    <table class="min-w-full text-sm mb-4">
                        ...
                        <tbody>
                            <tr v-for="(h, i) in advancedPaymentTarget.advanced_payment_histories" :key="h.id" class="cvr-table-row">
                                ...
                                <td class="px-3 py-2">
                                    <div class="flex items-center gap-2">
                                        <button v-if="permissions.canUpdate" @click="openEditAdvancedPayment(h)" class="cvr-btn-secondary inline-flex items-center px-2 py-1 rounded border text-xs">{{ $t('Edit') }}</button>
                                        <button v-if="permissions.canUpdate" @click="confirmDeleteAdvancedPayment(h)" class="cvr-btn-danger inline-flex items-center px-2 py-1 rounded border text-xs">{{ $t('Delete') }}</button>
                                    </div>
                                </td>
                            </tr>
