        ...
        <div class="p-6">
            <div class="flex items-center gap-3 mb-1">
                <Link :href="backUrl" class="cvr-btn-secondary inline-flex items-center gap-1 px-3 py-1.5 rounded border text-sm">
                    {{ $t('← Back to Overdraft Against Commercial Paper') }}
                </Link>
            </div>
            ...
            <div v-if="isLockedByRenewal" class="mb-4 px-4 py-3 rounded border border-blue-400 bg-blue-50 text-blue-800 text-sm">
                {{ $t('This facility has an active renewal, so this edits the') }} <strong>{{ $t('current chapter\'s') }}</strong> {{ $t('terms only. Account details, onboarding data (Outstanding Balance / Balance Date / Outstanding Breakdown), and the lending-rate tier schedule all belong to a specific chapter and can\'t be changed here — the tier schedule is set once, at the moment of a renewal, and applies to every cheque deposited under this chapter from then on. To change tiers or the renewal\'s own start date, delete and re-do the renewal from the Archived Facilities tab instead.') }}
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div v-if="!isLockedByRenewal" class="cvr-card">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-sm font-semibold cvr-text-secondary uppercase tracking-wide">{{ $t('Lending Information') }}</h2>
                    ...
                    <p class="text-xs cvr-text-muted mb-4">
                        {{ $t('Set a different lending rate depending on how many days remain until the commercial paper is due.') }}
                    </p>
                    <div v-for="row in lendingInformation" :key="row._rowId" class="cvr-form-grid-3 mb-2 items-end">
                        <div>
                            <label class="cvr-form-label">{{ $t('Commercial Papers Due Within (Days)') }}</label>
