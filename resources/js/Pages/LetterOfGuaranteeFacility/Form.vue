        ...
        <div class="p-6">
            <div class="flex items-center gap-3 mb-1">
                <Link :href="backUrl" class="cvr-btn-secondary inline-flex items-center gap-1 px-3 py-1.5 rounded border text-sm">
                    {{ $t('← Back to LG Facility') }}
                </Link>
            </div>
            <h1 class="text-xl font-semibold cvr-text-primary mb-1">
                {{ isEdit ? $t('Edit') : $t('Add') }} {{ $t('LG Facility') }}
            </h1>
            ...
            <div v-if="isLockedByRenewal" class="mb-4 px-4 py-3 rounded border border-blue-400 bg-blue-50 text-blue-800 text-sm">
                {{ $t('This facility has an active renewal, so this edits the') }} <strong>{{ $t('current chapter\'s') }}</strong> {{ $t('terms only — Limit and Contract End Date. The facility name, currency, start date, and Term &amp; Conditions matrix belong to a specific chapter and can\'t be changed here. To change those, delete and re-do the renewal from the Archived Facilities tab instead.') }}
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div class="cvr-card">
                    ...
                    <div class="cvr-form-grid-8-4 mb-3">
                        ...
                        <div>
                            <label class="cvr-form-label">{{ $t('LG Contract Name') }} *</label>
                ...
                <div v-if="!isLockedByRenewal" class="cvr-card">
                    <h2 class="text-sm font-semibold cvr-text-secondary uppercase tracking-wide mb-4">{{ $t('Term & Conditions — by LG Type') }}</h2>
                        ...
                        <table class="min-w-full text-sm">
                            ...
                            <tbody>
                                <tr v-for="row in termAndConditions" :key="row.lg_type" class="cvr-table-row">
                                    ...
                                    <td class="px-3 py-2">
                                        <div class="relative">
                                            <select v-model="row.commission_interval" class="cvr-input ps-2 pe-8 py-1.5 rounded w-full min-w-[11rem] appearance-none">
                                                <option v-for="(label, code) in commissionIntervals" :key="code" :value="code">{{ label }}</option>
                                            </select>
                                            <span class="pointer-events-none absolute end-2 top-1/2 -translate-y-1/2 text-xs cvr-text-muted">▾</span>
                                        </div>
                                    </td>
