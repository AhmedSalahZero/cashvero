<script setup>
/**
 * Statements/LgLcStatement/Result.vue
 * ------------------------------------------------------------------
 * Served by LGLCSBanktatementController@result. A running-balance
 * ledger for the chosen Letter of Credit / Letter of Guarantee / LC
 * Overdraft. Same "heavy report" treatment as Bank Statement: rows
 * are paginated SERVER-SIDE (50/page — the original never paginated
 * this report at all). No Reviewed column and no inline-editable
 * rows here, unlike Bank Statement — this report is read-only.
 */
...
const props = defineProps({
    company: Object,
    currency: String,
    isLcOverdraftBankStatement: Boolean,
    financialInstitutionName: String,
    letterOfCreditFacilityName: String,
    sourceLabel: String,
    typeLabel: String,
    kpis: Object,
    paginator: Object,
            ...
            <div class="flex items-start justify-between flex-wrap gap-2 mb-1">
                <h1 class="text-xl font-semibold cvr-text-primary">
                    ...
                    <span class="cvr-text-secondary font-normal">
                        ...
                        <template v-if="letterOfCreditFacilityName"> · {{ letterOfCreditFacilityName }}</template>
                    ...
                    <table class="min-w-full text-sm">
                        <thead class="cvr-table-head sticky top-0 z-10">
                            <tr>
                                ...
                                <th v-if="isLcOverdraftBankStatement" class="px-3 py-3 text-right">{{ $t('Limit') }}</th>
                                ...
                                <th v-if="isLcOverdraftBankStatement" class="px-3 py-3 text-right">{{ $t('Room') }}</th>
                        ...
                        <tbody>
                            <tr v-for="(row, index) in rows" :key="row.id" class="cvr-table-row">
                                ...
                                <td v-if="isLcOverdraftBankStatement" class="px-3 py-2.5 text-right cvr-num">{{ formatAmount(row.limit) }}</td>
                                ...
                                <td v-if="isLcOverdraftBankStatement" class="px-3 py-2.5 text-right cvr-num">{{ formatAmount(row.room) }}</td>
                                <td class="px-3 py-2.5 text-start cvr-text-secondary">
                                    <span class="block max-w-[360px] whitespace-normal">{{ row.comment }}</span>
                                </td>
                            </tr>
                            <tr v-if="rows.length === 0">
                                <td colspan="9" class="px-4 py-8 text-center cvr-text-muted">{{ $t('No movements found for this date range.') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
