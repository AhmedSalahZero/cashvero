 ...
 * ------------------------------------------------------------------
 * Served by FactoringChargesStatementController@index. Filter form:
 * date range, then Factoring Company → Currency (cascading), then an
 * OPTIONAL Factoring Contract (cascading too) — leaving it blank means
 * "every contract for this company in this currency," matching the
 * original's "All Contracts" placeholder.
 *
 * The second tab of the Factoring Statement page.
 */
...
const props = defineProps({
    ...
    urls: Object, // { result, currencies, contracts, statementUrl }
});
...
const canSubmit = computed(() =>
    startDate.value && endDate.value && endDate.value <= maxDate && factoringCompanyId.value && currency.value
);
...
<template>
    <AppLayout>
        <div class="p-6 max-w-7xl">
            <h1 class="text-xl font-semibold cvr-text-primary mb-1">{{ $t('Factoring Charges Statement') }}</h1>
            <p class="text-sm cvr-text-muted mb-4">
                {{ $t('Every factoring charge (interest, other charges, uncollected invoice charges), with a running total.') }}
            </p>
            ...
            <div class="cvr-card-bg cvr-border border rounded-lg p-5">
                ...
                <div class="cvr-form-grid-3">
                    ...
                    <div>
                        ...
                        <select v-model="factoringContractId" class="cvr-input w-full px-3 py-2 rounded" :disabled="loadingContracts">
                            <option value="">{{ loadingContracts ? $t('Loading…') : $t('All Contracts') }}</option>
