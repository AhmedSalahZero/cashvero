...
<template>
    <div class="print-page">
        ...
        <section class="sheet">
            <header class="header">
                <div>
                    <h1>{{ $t('Sell Or Buy Currencies') }}</h1>
                    <p>{{ company.name }}</p>
                </div>
                <div class="meta">
            ...
            <section class="summary">
                ...
                <div><span>{{ $t('Transaction Date') }}</span><strong>{{ record.date }}</strong></div>
                <div><span>{{ $t('Amount To Sell') }}</span><strong class="num">{{ record.amount_to_sell }} {{ record.currency_to_sell }}</strong></div>
                <div><span>{{ $t('Exchange Rate') }}</span><strong class="num">{{ record.exchange_rate }}</strong></div>
                <div><span>{{ $t('Amount To Buy') }}</span><strong class="num">{{ record.amount_to_buy }} {{ record.currency_to_buy }}</strong></div>
            </section>
            ...
            <section v-if="record.user_comment" class="block">
                <h2>{{ $t('User Comment') }}</h2>
                <p class="comment">{{ record.user_comment }}</p>
            </section>
        </section>
...
.table th { width: 260px; background: #f8fafc; }
.comment { margin: 0; border: 1px solid #e5e7eb; background: #f8fafc; border-radius: 8px; padding: 10px; white-space: pre-wrap; }
