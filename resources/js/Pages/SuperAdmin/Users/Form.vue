    ...
    <AppLayout>
        <div class="p-6">
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div class="cvr-card">
                    <h2 class="text-sm font-semibold cvr-text-secondary uppercase tracking-wide mb-4">{{ $t('Users') }}</h2>
                ...
                <div class="cvr-card">
                    <h2 class="text-sm font-semibold cvr-text-secondary uppercase tracking-wide mb-4">{{ $t('User Information') }}</h2>
                    <div class="cvr-form-grid-2 mb-4">
                        <div>
                            <label class="cvr-form-label">{{ $t('Email') }} *</label>
                        ...
                        <div>
                            <label class="cvr-form-label">{{ $t('User Image') }} *</label>
                            <input type="file" @change="onAvatarChange" class="cvr-input w-full px-3 py-2 rounded" />
                            <img v-if="model?.avatar_url" :src="model.avatar_url" class="w-16 h-16 object-cover rounded-full mt-2" :alt="$t('current avatar')" />
                        </div>
                    </div>
                    <div v-if="!isEdit" class="cvr-form-grid-2">
                        <div>
                            <label class="cvr-form-label">{{ $t('Password') }} *</label>
                        ...
                        <div>
                            <label class="cvr-form-label">{{ $t('Confirm Password') }} *</label>
                ...
                <div class="cvr-card">
                    <h2 class="text-sm font-semibold cvr-text-secondary uppercase tracking-wide mb-4">{{ $t('Assign Companies To This User') }}</h2>
                    <p v-if="!canEditCompanies" class="text-xs cvr-text-secondary mb-3">
                        {{ $t('Company assignment is locked to your company. Only a Super Admin can change it.') }}
                    </p>
                    <div class="cvr-form-grid-2">
                        <div>
                            <label class="cvr-form-label">{{ $t('Select Companies — (Multi Selection)') }} *</label>
                        ...
                        <div>
                            <label class="cvr-form-label">{{ $t('Role') }} *</label>
                            ...
                            <p v-if="!isEdit" class="text-xs cvr-text-muted mt-1.5">
                                {{ $t('The role\'s permission template is copied to this user on save. You can fine-tune it afterwards from the Users list.') }}
                            </p>
                            ...
                            <label v-if="roleChanged" class="flex items-start gap-2 mt-3 cursor-pointer">
                                ...
                                <span class="text-xs cvr-text-secondary">
                                    {{ $t('Replace this user\'s permissions with the new role\'s template.') }}
                                    <span class="cvr-text-muted">
                                        {{ $t('Leave unticked to keep their current permissions exactly as they are — the role is only a label unless you tick this.') }}
                                    </span>
                                </span>
                            </label>
                            <div v-if="showMaxUsers" class="mt-3">
                                <label class="cvr-form-label">{{ $t('Max Users Allowed') }} *</label>
