...
@endphp
@section('sub-header')
{{ $isEdit ? __('Edit Factoring Without Recourse') : __('Create Factoring Without Recourse') }}
@endsection
@section('content')
<div class="money-flow-dark" id="factoring-without-recourse-form"
    ...
    @endif
    data-contracts-url="{{ url(app()->getLocale() . '/' . $company->id . '/factoring/without-recourse/contracts') }}"
    data-currencies-url="{{ url(app()->getLocale() . '/' . $company->id . '/factoring/without-recourse/currencies') }}"
    data-invoices-url="{{ url(app()->getLocale() . '/' . $company->id . '/factoring/without-recourse/invoices') }}"
    data-calculate-url="{{ route('factoring.without-recourse.calculate', ['company' => $company->id]) }}"
    data-account-numbers-url="{{ url(app()->getLocale() . '/' . $company->id . '/money-received/get-account-numbers-based-on-account-type') }}">
<div class="row">
    <div class="col-md-12">
        <form method="post"
            action="{{ $isEdit ? route('factoring.without-recourse.update', ['company' => $company->id, 'factoringTransaction' => $factoringTransaction->id]) : route('factoring.without-recourse.store', ['company' => $company->id]) }}"
                ...
                <div class="kt-portlet__head">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title head-title text-primary">
                            <x-sectionTitle :title="$isEdit ? __('Edit Factoring Without Recourse') : __('Create Factoring Without Recourse')"></x-sectionTitle>
                        </h3>
                    </div>
                </div>
            ...
            <x-submitting-by-ajax :backTo="route('factoring.without-recourse.index', ['company' => $company->id])" />
        </form>
    </div>
</div>
</div>
...
<script src="{{ url('custom/factoring-without-recourse.js') }}?v={{ time() }}"></script>
@endsection
