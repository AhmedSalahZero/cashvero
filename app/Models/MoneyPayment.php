...
class MoneyPayment extends Model implements IHaveCreditOverdraftStatement, IHaveMediumTermLoanCreditStatement, IHaveLeasingContractCreditStatement
{
	...
	public static function scopeFilterByDeliveryDate( $query, ?string $startDate = null, ?string $endDate = null)
	{
     ...
     */
    public function settlementsForMoneyPayment():HasMany
    {
    ...
    public function payableCheque():HasOne
    {
     ...
     *
     * * الشرط كان بيقارن type (نوع الصرف : cash-in-safe /
     * * outgoing-transfer / cheque ...) بـ 'is_supplier' — قيمة من
    ...
    public static function requestHasInvoiceSettlementWithDownPayment(\Illuminate\Http\Request $request): bool
    {
        return $request->get('unapplied_amount', 0) > 0
            ...
            && $request->get('partner_type', 'is_supplier') === 'is_supplier';
    }
    ...
    public function getOdooIdWithRefOfTransaction():array
    {
        ...
        return [
			'id'=>$odooSettings->advances_to_suppliers_id,
			'ref'=>'Down Payment'
		];
