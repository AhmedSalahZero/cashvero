				...
				delimiter // 
				create procedure resettlement_overdraft_against_assignment_of_contract_from(in _type varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci, in _overdraft_against_assignment_of_contract_id integer , in _current_company_id integer  )
				begin 
 ...
 declare _total_month_interest_amount decimal(14,2) default 0 ;
		...
		select sum(interest_amount)  into _total_month_interest_amount from   overdraft_against_assignment_of_contract_bank_statements where id!= new.id and company_id = new.company_id and overdraft_against_assignment_of_contract_id = new.overdraft_against_assignment_of_contract_id and month(date) = month(new.date) and year(date) = year(new.date) ; 
		...
		if(new.interest_type = 'end_of_month') then  
			...
			--				select start_settlement_from_bank_statement_date into _last_bank_statement_date_to_start_settlement_from from overdraft_against_assignment_of_contracts where id = new.overdraft_against_assignment_of_contract_id ; 
				...
				create procedure start_settlement_process_overdraft_against_contract(in _type varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci ,in _bank_statement_id integer , in _overdraft_against_assignment_of_contract_id integer , in _debit decimal , in _credit decimal , in _company_id integer , in _date_for_settlement date)
